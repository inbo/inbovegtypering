...
#'
#' Calculates similarity indices (Weirdness, Incompleteness, Likelihood, MED, CoD)
#' for vegetation recordings against a synoptic table of syntaxa.
#'
#' @param data A data frame containing the vegetation recordings. Must contain columns:
...
#'   If a data frame, it must contain columns: `syntaxon_code`, `species_number`,
...
#' @param layer Character string indicating the layer code to filter from `data`.
...
#'
#' @importFrom dplyr filter mutate group_by summarise transmute left_join do ungroup
#' @importFrom tidyr crossing replace_na
#' @importFrom purrr map_int
#' @importFrom assertthat assert_that has_name is.readable
#' @importFrom utils read.csv
#' @export
classify_vegetation <- function(data, synoptics, layer = "ALL", r = 0.50, s = 0.60, t = 0.50) {
  # --- Assertions (Inputs) ---
  assertthat::assert_that(
    is.data.frame(data),
    assertthat::has_name(data, c("recording", "species_number", "pct_value", "layer_code")),
  ...
  # This ensures we have rows for species absent in the recording but present in syntaxon
